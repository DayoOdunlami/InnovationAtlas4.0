...
describe.skipIf(!hasRealPostgresUrl())(
  "message-repository (permit/deny)",
  () => {
    ...
    beforeAll(async () => {
      ...
      ({ pgBriefRepository: briefRepo } = await import(
        "./brief-repository.pg"
      ));
    ...
    afterAll(async () => {
      ...
      if (createdTokenIds.length > 0) {
        await db
          ...
          .where(
            inArray(schema.AtlasBriefShareTokensTable.id, createdTokenIds),
          );
      }
    ...
    async function newBrief() {
      const brief = await briefRepo.createBrief(
        {
          ownerId,
          title: `msg-repo-test-${Math.random().toString(36).slice(2, 8)}`,
        },
    ...
    async function newShareToken(
      briefId: string,
      opts?: { revoked?: boolean; expiresInMs?: number },
    ) {
